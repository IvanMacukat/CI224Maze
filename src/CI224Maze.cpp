...
const GLint WINDOW_STARTX=175;
const GLint WINDOW_STARTY=150;
...
// Textures used for the walls and sky background
#define TEXTURE_FILE "wall.bmp"
#define SKY_FILE "sky.bmp"

// The maze compile time parameters
const GLint XSIZE=8;
const GLint YSIZE=8;
// Game level Difficulty. Higher values = More turns in CORRECT PATH!
const GLint DIFFICULTY=2;

// The amount of times the game attempts to create a "false path" coordinate
...
// Returns a pointer to the variable portion of the maze, in 2D format.
GLint ((*(mazeInnards()))[YSIZE])
{
    ...
    /* "Innards" exclude the outer wall... it's worth noting that the array mapping
       function inherent to the initialisation just above is completely
...
void appAssertSuccess(const char* szz)
{
...
GLuint maketex(const char* tfile,GLint xSize,GLint ySize) //returns tex. no.
{
    ...
    appAssertSuccess("post0_image");

    glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER, GL_LINEAR);

    appAssertSuccess("pre_getview");
    ...
    appAssertSuccess("pre_view");
    glViewport(0,0,xSize,ySize);
    appAssertSuccess("post0_view");
...
void makeSolution() //We don't use that extra +1 in the Y-dimension
{
    ...
    while(x<XSIZE)
    {
        ...
        do
        {
            ...
            /* Set path_leg_length... must be at l**east 3 or we will make
...
bool validForObfuscation(int x, int y)
{
    // Check ranges
    if(x<=0) return false;
    if(y<0) return false;
    ...
    if((mazeInnards())[x][y]) return false;
    ...
    if((mazeInnards())[x+1][y]) ++ret;
    if(x-1>=0 && (mazeInnards())[x-1][y]) ++ret;
    if(y+1<YSIZE && (mazeInnards())[x][y+1]) ++ret;
    if(y-1>=0 && (mazeInnards())[x][y-1]) ++ret;
...
void obfuscateMaze()
{
    ...
    for(int ob=0; ob < OBFUSCATION_LOOP_RUNS; ++ob)
    {
        ...
        if(validForObfuscation(x,y))
        {
            c++;
            (mazeInnards())[x][y]=FALSE_PATH;
        }
    }
}

void printTheMaze() //Renders the necessary OpenGL cubes
{
    int x,y;
...
bool collision() //Is player in a state of collision?
{
    int x,y;
...
// Move, incorporating collision and bounceback hit detection
void movement(GLfloat amt)
{
    x_at+=cos(rot)*amt;
    y_at+=sin(rot)*amt;
    if(collision()) //Don't let player walk through walls
    {
        x_at-=BOUNCEBACK*cos(rot)*amt;
        y_at-=BOUNCEBACK*sin(rot)*amt;
    }
    if(collision()) //Bounced into another wall... just reverse original move
    {
        x_at+=BOUNCEBACK*cos(rot)*amt;
        y_at+=BOUNCEBACK*sin(rot)*amt;
        x_at-=cos(rot)*amt;
...
void displayFunction()
{
    ...
    if(camera_y<=0.0f && xin && yin)
    {
        if(yin<CONTROLLER_PLAY)
            movement((yin-windowHeight()/2.0f)*-WALK_MOUSE_SENSE);
        if(yin>(windowHeight()-CONTROLLER_PLAY))
            movement(((windowHeight()/2.0f)-yin)*WALK_MOUSE_REVERSE_SENSE);
...
void arrowsKeys(GLint key, GLint x, GLint y)
{
    if(key == GLUT_KEY_UP)
        movement(WALK_KEY_SENSE);
    if(key == GLUT_KEY_DOWN)
        movement(-WALK_KEY_REVERSE_SENSE);
...
{
    // Used to wait for a real mouse move vs. initial positioning call
    static int mouses=0;
    if(mouses<=1)
    {
        ++mouses;
...
int main(int argc, char** argv) {
    ...
    gluPerspective(VIEW_FIELD,(GLfloat)windowWidth()/(GLfloat)windowHeight(),NEAR_Z,FAR_Z);
